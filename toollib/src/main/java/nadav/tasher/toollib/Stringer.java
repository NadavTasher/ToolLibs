...
public class Stringer {
	public static ArrayList<String> cutOnEvery(String s, String xc) {
	...
	public static String replaceEvery(String s, char toreplace, char replaced) {
		String f="";
					...
					for(int c=0; c<all.length(); c++){
						if(!tv.getText().toString().equals(STOP_ANIMATION)){
							...
							a.runOnUiThread(new Runnable() {
								@Override
								public void run() {
									String txt=tv.getText().toString() + all.charAt(finalC);
									tv.setText(txt);
								}
							});
							try{
